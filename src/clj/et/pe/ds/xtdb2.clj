        ...
        (if (and existing (not= (:id existing) id))
          ...
          (let [raw (first (xt/q (:conn conn)
            ...
            (xt/execute-tx (:conn conn) [[:put-docs :personas {:xt/id                id
                                                               ...
                                                               :persona/password-hash (:persona/password-hash raw)}]])
            {:success true}))))))
...
(defn- make-identity-id [persona-id id]
  (keyword (str (clojure.core/name persona-id) "/" (clojure.core/name id))))

(defn- extract-identity-id [composite-id]
  (let [s (clojure.core/name composite-id)
        idx (.lastIndexOf s "/")]
...
(defn- make-relation-id [persona-id source-id target-id]
  (keyword (str (clojure.core/name persona-id) "/" (clojure.core/name source-id) "/" (clojure.core/name target-id))))
...
(defn delete-relation
  ...
  (let [full-id (keyword (str (clojure.core/name persona-id) "/" relation-id))]
    (xt/execute-tx (:conn conn)
                   [(if valid-from
